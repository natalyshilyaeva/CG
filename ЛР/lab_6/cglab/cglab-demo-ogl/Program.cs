...
public abstract class CGLabDemoOGL : OGLApplicationTemplate<CGLabDemoOGL>
{
    ...
    public void Create_Vertex()
    {
        ...
        for (int j = 1; j < Height; j += numbers[Aprox], TopCircles++)
        {
            ...
            for (int phi = 0; phi <= 360; phi += numbers[Aprox], CntVertInTopCircles++)
            {
                ...
                Z = (Height / 2);
                vertices[iterator] = new Vertex((float)X, (float)Y, (float)Z, 0, 0, 0, Math.Abs((float)Math.Sin(cur_time)), Math.Abs((float)Math.Sin(cur_time)), Math.Abs((float)Math.Sin(cur_time)));
                iterator++;
            }
        }
        ///////////////////////////////////////////////////////
        ...
        vertices[iterator] = new Vertex((float)0, (float)0, (float)(Height / 2), 0, 0, 0, Math.Abs((float)Math.Sin(cur_time)), Math.Abs((float)Math.Sin(cur_time)), Math.Abs((float)Math.Sin(cur_time)));
        iterator++;
        vertices[iterator] = new Vertex((float)0, (float)0, (float)(-Height / 2), 0, 0, 0, Math.Abs((float)Math.Sin(cur_time)), Math.Abs((float)Math.Sin(cur_time)), Math.Abs((float)Math.Sin(cur_time)));
        iterator++;
    }
    ...
    public void Rotation()
    {
        if (EnableRot)
        {
            cameraAngle += new DVector3(0, 0.1, 0);
        }
    }
    ...
    public void Shader(OGLDeviceUpdateArgs e, int offset_vx, int offset_nx, int mode)
    {
        unsafe
        {
            ...
            gl.Uniform3(objectColorLoc, (float)ObjectColor.X * Math.Abs( (float)Math.Sin(cur_time)), (float)ObjectColor.Y * Math.Abs((float)Math.Sin(cur_time)), (float)ObjectColor.Z * Math.Abs((float)Math.Sin(cur_time)));
            gl.Uniform3(lightColorLoc, (float)LightColor.X /** Math.Abs( (float)Math.Sin(cur_time - 1))*/, (float)LightColor.Y /** Math.Abs((float)Math.Sin(cur_time - 1))*/, (float)LightColor.Z /** Math.Abs((float)Math.Sin(cur_time - 1))*/);
