...
{
    #region элементы GUI
    [DisplayNumericProperty(0.5, 0.1, "a", 0.1, 100)]
    ...
    [DisplayNumericProperty(10, 0.01, "B", 0.01, 15)]
    ...
    [DisplayNumericProperty(0.01, 0.01, "Шаг", 0.01, 10)]
    ...
    protected override void OnDeviceUpdate(object s, DeviceArgs e)
    {
        ...
        e.Graphics.DrawString("Вариант 20\ny=sqrt((x^3)/(a-x)), 0<x<=B", new Font("Arial", 10f), Brushes.Black, 10f, 10f);
    ...
    void CalculatePoints()
    {
        ...
        lock (RenderDevice.LockObj) // чтобы не изменить список во время обращения к нему из другого потока
        {
            ...
            for (double x = B; x >= 0; x -= step)
            {
                if (a - x == 0) continue;
                if (Math.Pow(x, 3) / (a - x) < 0) continue;
                points.Add(new DVector2(x, -Math.Sqrt(Math.Pow(x, 3) / (a - x))));
