...
public abstract class CGLabDemoOGL : OGLApplicationTemplate<CGLabDemoOGL>
{
    ...
    //[DisplayNumericProperty(100, 1, "Высота", 1)]
    ...
    [DisplayNumericProperty(new[] { 0.0d, 0d, 0.5d }, 0.01, "Kd", 0, 1)]
    ...
    public void Create_Vertex()
    {
        ...
        for (int j = 1; j < Height; j += numbers[Aprox], TopCircles++)
        {
            ...
            for (int phi = 0; phi <= 360; phi += numbers[Aprox], CntVertInTopCircles++)
            {
                ...
                Z = (Height / 2);
                vertices[iterator] = new Vertex((float)X, (float)Y, (float)Z, 0, 0, 0, Math.Abs((float)Math.Cos(cur_time)), Math.Abs((float)Math.Cos(cur_time)), Math.Abs((float)Math.Cos(cur_time)));
                iterator++;
            }
        }
        ///////////////////////////////////////////////////////
        ...
        //{
        //    vertices[visited + i * CntVertInCircle + CntVertInCircle - 1] = vertices[visited + i * CntVertInCircle];
        //}

        ////////////////////////////////////////////////////центры верхней и нижней крышки
        //if (Circles == 4)
        //Environment.Exit(0);
        vertices[iterator] = new Vertex((float)0, (float)0, (float)(Height / 2), 0, 0, 0, Math.Abs((float)Math.Cos(cur_time)), Math.Abs((float)Math.Cos(cur_time)), Math.Abs((float)Math.Cos(cur_time)));
        iterator++;
        vertices[iterator] = new Vertex((float)0, (float)0, (float)(-Height / 2), 0, 0, 0, Math.Abs((float)Math.Cos(cur_time)), Math.Abs((float)Math.Cos(cur_time)), Math.Abs((float)Math.Cos(cur_time)));
        iterator++;
    }
    ...
    public void Create_Polygons()
    {
        ...
        //for (int i = 0; i < Circles; i++)
        //{
        //    Vertex tmp = vertices[CntVertInCircle * i + 4 + visited];
        //    tmp = new Vertex(tmp.vx, tmp.vy, tmp.vz, 1, 0, 0, tmp.r, tmp.g, tmp.b);
        //}
    }
    ...
    public void Shader(OGLDeviceUpdateArgs e, int offset_vx, int offset_nx, int mode)
    {
        unsafe
        {
            ...
            gl.Uniform3(objectColorLoc, Math.Abs((float)Math.Cos(cur_time)), Math.Abs((float)Math.Cos(cur_time)), Math.Abs((float)Math.Cos(cur_time)));
            gl.Uniform3(lightColorLoc, Math.Abs((float)Math.Cos(cur_time - 1)), Math.Abs((float)Math.Cos(cur_time - 1)), Math.Abs((float)Math.Cos(cur_time - 1)));
          ...
          // gl.Uniform1(intime, cur_time);
            ...
            gl.Uniform3(AmbientPosLoc, (float)Math.Cos(cur_time), (float)Math.Cos(cur_time), (float)Math.Cos(cur_time));
            gl.Uniform3(DiffusePosLoc, (float)Math.Cos(cur_time), (float)Math.Cos(cur_time), (float)Math.Cos(cur_time));
            gl.Uniform3(SpectrPosLoc, (float)Math.Cos(cur_time), (float)Math.Cos(cur_time), (float)Math.Cos(cur_time));
